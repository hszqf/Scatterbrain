...
19. replay 中 `queue_update` 的播放时机必须晚于其触发源 board item（`move/ghostify/beat_empty`），且早于 `queue_restart` 后的下一轮 board replay/reset：先播触发源 board，再播该次 queue_update（append/evict/settle），再进入 queue_restart 与下一轮；禁止 queue_update 先行或拖到下一轮导致视觉因果倒置。
